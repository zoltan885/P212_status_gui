
import logging
import sys
import threading
import asyncio
import time



from poller_async import AsyncPoller
from poller_simulator import AsyncSensorSimulator, SENSOR_CONFIG
from current_state_async import CurrentStateMonitorAsync, OverwritingDualModeQueue

import importlib
if len(sys.argv) > 1:
    conf_name = sys.argv[1]
    conf = importlib.import_module(conf_name)
    logging.info(f'{sys.argv[1]} imported')
else:
    import configuration as conf

logFormatter = logging.Formatter(
    "%(asctime)-25.25s %(threadName)-12.12s %(name)-25.24s %(levelname)-10.10s %(message)s")
rootLogger = logging.getLogger()
rootLogger.setLevel(logging.INFO)

consoleHandler = logging.StreamHandler(sys.stdout)
consoleHandler.setFormatter(logFormatter)
rootLogger.addHandler(consoleHandler)


async def drain_update_queue_nonblocking(queue: asyncio.Queue):
    """Drain all currently available items without waiting for more."""
    items = []
    while True:
        try:
            item = queue.get_nowait()
        except asyncio.QueueEmpty:
            break
        else:
            items.append(item)
            if isinstance(queue, OverwritingDualModeQueue):
                pass
            else:
                queue.task_done()
    if items:
        print(f"UPDATE: {len(items)} items: {items}")

async def drain_snapshot_queue_nonblocking(queue: asyncio.Queue):
    """Drain all currently available items without waiting for more."""
    items = []
    while True:
        try:
            item = queue.get_nowait()
        except asyncio.QueueEmpty:
            break
        else:
            items.append(item)
            if isinstance(queue, OverwritingDualModeQueue):
                pass
            else:
                queue.task_done()
    if items:
        print(f"\nSNAPSHOT: {len(items)}\nitems: {items}\n")


def background_worker(queue: asyncio.Queue, loop: asyncio.AbstractEventLoop, typ: str = "update"):
    """Periodically drain the queue from a background thread."""
    while True:
        # Schedule the async drain in the main loop
        if typ == "update":
            fut = asyncio.run_coroutine_threadsafe(
                drain_update_queue_nonblocking(queue), loop
            )
        elif typ == "snapshot":
            fut = asyncio.run_coroutine_threadsafe(
                drain_snapshot_queue_nonblocking(queue), loop
            )

        fut.result()  # Wait for completion
        # Simulate periodic check
        import time
        time.sleep(0.1)

SIMULATE = False

async def main():
    queue = asyncio.Queue()
    if SIMULATE:
        poller = AsyncSensorSimulator(SENSOR_CONFIG, queue)
    else:
        # Use the actual AsyncPoller if not simulating
        logging.info("Using actual AsyncPoller")
        poller = AsyncPoller(queue = queue)

    update_queue = asyncio.Queue()
    snapshot_queue = asyncio.Queue()# OverwritingDualModeQueue()
    
    print("Setting up Queue drain worker...")
    try:
        loop = asyncio.get_running_loop()
    except RuntimeError:
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
    threading.Thread(target=background_worker, args=(update_queue, loop), kwargs={'typ': 'update'}, daemon=True).start()
    threading.Thread(target=background_worker, args=(snapshot_queue, loop), kwargs={'typ': 'snapshot'}, daemon=True).start()

    if not SIMULATE:
        for tab in conf.grouping['tabs']:
            for gr in conf.grouping['tabs'][tab].keys():
                for coll in conf.grouping['tabs'][tab][gr]:
                    for k, v in getattr(conf, coll).items():
                        if 'attr' in v.keys():
                            await poller.add_attr(v, state=True, name=k)
    
    await poller.start()
    print("Poller started...")

    CSMA = CurrentStateMonitorAsync(
        in_queue=queue, 
        update_queue=update_queue, 
        snapshot_queue=snapshot_queue
    )

    CSMA.snapshot_interval = 10

    print("Starting CurrentStateMonitorAsync...")

    await CSMA.start_update()
    #await CSMA.start_snapshot()

    print("CurrentStateMonitorAsync started...")
    
    sleep_time = 5
    print(f"Sleeping for {sleep_time} seconds...")
    await asyncio.sleep(sleep_time/2)
    print('\n\nON DEMAND SNAPSHOT')
    await CSMA.snapshot()
    print('\n\n')
    await asyncio.sleep(sleep_time/2)
    print("Stopping CurrentStateMonitorAsync...")
    try:
        await CSMA.stop_update()
        await CSMA.stop_snapshot()
    except Exception as e:
        pass
    print("CurrentStateMonitorAsync stopped...")
    print('Getting a snapshot...')
    await CSMA.snapshot()

    print("Stopping poller...")
    await poller.stop() 
    print("Poller stopped...")
    print("All tasks completed, exiting...")

if __name__ == "__main__":  
    logging.basicConfig(level=logging.DEBUG)
    try:
        asyncio.run(main())
    except KeyboardInterrupt:
        logging.info("KeyboardInterrupt received, stopping...")
    except Exception as e:
        logging.error(f"An error occurred: {e}")



